...
function wakeup()
	...
	TriggerServerEvent('grp_Sit:leavePlace', currentSitCoords)
...
function sit(object, modelName, data)
	...
	ESX.TriggerServerCallback('grp_Sit:getPlace', function(occupied)
		if occupied then
			ESX.ShowNotification('Hier sitzt bereits jemand.')
		else
			local playerPed = PlayerPedId()
			lastPos, currentSitCoords = GetEntityCoords(playerPed), objectCoords

			TriggerServerEvent('grp_Sit:takePlace', objectCoords)
