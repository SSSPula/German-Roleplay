...
RegisterNetEvent('grp_TV:update')
AddEventHandler('grp_TV:update', function(players)
...
CreateThread(function()
    ...
    TriggerServerEvent('grp_TV:fetch')
...
ESX.TriggerServerCallback('grrasdda:welchdonohastdu', function (okay)
	...
	if args[1] then
        for k, v in pairs(Config['Objects']) do
            ...
            if DoesEntityExist(obj) then
                TriggerServerEvent('grp_TV:add', args[1], v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'])
                break
            end
        end
    end
...
RegisterCommand('lautst', function(src, args)
    ESX.TriggerServerCallback('grd_Pets:Gruppegetten', function(plyGroup)
	if plyGroup == 'skindonator' or plyGroup == 'superadmin' or plyGroup == 'skindonator' or plyGroup == 'tiermarktdonator' or plyGroup == 'tierdonator' or plyGroup == 'marktdonator' or plyGroup == 'skintierdonator' or plyGroup == 'admin' or plyGroup == 'skinmarktdonator' or plyGroup == 'skintiermarktdonator' then
	if args[1] then
            ...
            if volume >= 0 then
                ...
                for k, v in pairs(data) do
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                        ...
                        TriggerServerEvent('grp_TV:setvolume', k, volume)
                        break
                    end
                end
	...
	exports['mythic_notify']:DoHudText('error', 'Du musst Unterstützer sein. Infos sind im Discord.!', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
	end
	end)
end)

RegisterCommand('tvaus', function(src, args)
	ESX.TriggerServerCallback('grd_Pets:Gruppegetten', function(plyGroup)
	if plyGroup == 'skindonator' or plyGroup == 'superadmin' or plyGroup == 'skindonator' or plyGroup == 'tiermarktdonator' or plyGroup == 'tierdonator' or plyGroup == 'marktdonator' or plyGroup == 'skintierdonator' or plyGroup == 'admin' or plyGroup == 'skinmarktdonator' or plyGroup == 'skintiermarktdonator' then

    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
            TriggerServerEvent('grp_TV:destroy', k)
            break
        end
    end
	else
...
RegisterNetEvent('grp_TV:delete')
AddEventHandler('grp_TV:delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('grp_TV:updatevolume')
AddEventHandler('grp_TV:updatevolume', function(id, volume)
    if data[id] then
