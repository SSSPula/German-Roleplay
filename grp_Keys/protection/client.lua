                    ...
                    function(data2, menu2)
                            if(data2.current.value == 'give_key_accept') then
                                    ...
                                    if distance ~= -1 and distance <= 2.0 then
                                            TriggerServerEvent('grp_keys:lendCarKeys', GetPlayerServerId(player), data2.current.plate)
									else
...
function ToggleVehicleLock()
    ...
    if DoesEntityExist(car) then
            ESX.TriggerServerCallback('grp_keys:fetchCarKey', function(hasKey)
                    ESX.TriggerServerCallback('grp_keys:fetchCarWlKey', function(hasWlKey)
...
RegisterNetEvent('grp_keys:syncTableKeys')
AddEventHandler('grp_keys:syncTableKeys', function(keysData, identifier)
    carKeys = keysData
