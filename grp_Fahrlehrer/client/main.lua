...
function StartTheoryTest()
	...
	--TriggerServerEvent('grp_Fahrlehrer:pay', Config.Prices['dmv'])
end
...
function StopTheoryTest(success)
	...
	if success then
		TriggerServerEvent('grp_Fahrlehrer:GibScheinHer', 'dmv')
...
function StartDriveTest(type)
	...
	--TriggerServerEvent('grp_Fahrlehrer:pay', Config.Prices[type])
end

function StopDriveTest(success)
	if success then
		TriggerServerEvent('grp_Fahrlehrer:GibScheinHer', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
RegisterNetEvent('grp_Fahrlehrer:ofeffeeenne')
AddEventHandler('grp_Fahrlehrer:ofeffeeenne', function()
...
Citizen.CreateThread(function()
    ...
    exports['bt-target']:AddTargetModel(fahrlehrraa, {
        options = {
            {
                event = "grp_Fahrlehrer:ofeffeeenne",
                icon = "fas fa-car",
                label = "Führerschein machen",
            },
        },
        job = {"all"},
        distance = 2.5
...
AddEventHandler('grp_Fahrlehrer:hasEnteredMarker', function(zone)
...
AddEventHandler('grp_Fahrlehrer:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('grp_Fahrlehrer:loadLicenses')
AddEventHandler('grp_Fahrlehrer:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('grp_Fahrlehrer:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('grp_Fahrlehrer:hasExitedMarker', LastZone)
		end
	end
end)
