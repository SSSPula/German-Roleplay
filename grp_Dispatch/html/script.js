...
$(document).keyup(function (e) {
  if (e.keyCode === 27) {
    ...
    $.post('https://grp_Dispatch/close', JSON.stringify({}));
...
function forwardCall(num) {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == num) {
      $.post('https://grp_Dispatch/forwardCall', JSON.stringify({
...
function copyNumber() {
  ...
  $.post('https://grp_Dispatch/copynumber', JSON.stringify({}));
}
...
function togglecallblips() {
  $.post('https://grp_Dispatch/togglecallblips', JSON.stringify({}));
}

function toggleUnitBlips() {
  $.post('https://grp_Dispatch/toggleunitblips', JSON.stringify({}));
}

function toggleAlerts() {

  $.post('https://grp_Dispatch/togglealerts', JSON.stringify({}));

}

function changeToBusy() {
  $.post('https://grp_Dispatch/updatestatus', JSON.stringify({
    id: id,
    status: 'Beschäftigt'
  }));
}
...
function dismissCall() {
  ...
  acceptedCall = '';
  $.post('https://grp_Dispatch/dismissCall', JSON.stringify({
    id: currentElement.id
  }));
...
function removeCall() {
  ...
  $.post('https://grp_Dispatch/removeCall', JSON.stringify({
    id: currentElement.id
  }));
...
function acceptCall() {
  ...
  $.post('https://grp_Dispatch/acceptCall', JSON.stringify({
    id: currentElement.id,
  ...
  $.post('https://grp_Dispatch/sendnotice', JSON.stringify({
...
function requestBackup() {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == col) {
      $.post('https://grp_Dispatch/reqbackup', JSON.stringify({
        job: key,
  ...
  if (element != null) {
    ...
    if (columnMenu) {
      ...
      if (jobInfo[job].canRequestLocalBackup && col == jobInfo[job].column) {
        html = html + '    <a onclick="requestBackup()">Verstärkung anfordern</a>';
      }
      ...
      html = html + '    <a onclick="changeToBusy()">Status: Beschäftigt</a>';
      html = html + '    <a onclick="changeToPatrol()">Status: Auf Streife</a>';


    }
    else {
      if (acceptedCall == '') {
        html = html + '    <a onclick="acceptCall()">Aktzeptiere Dispatch</a>';
        if (jobInfo[job].canRemoveCall) {
          html = html + '    <a onclick="removeCall()">Entferne Dispatch</a>';
        }
        if ($(element).hasClass('message')) {
          html = html + '    <a onclick="copyNumber()">Kopiere Nummer</a>';
        }

      }

      if (acceptedCall != '' && acceptedCall == $(element).attr('id')) {
        html = html + '    <a onclick="dismissCall()">Entferne Dispatch</a>';
      }


      for (const [key, value] of Object.entries(jobInfo)) {
        if (key != job && jobInfo[job].forwardCall) {
          html = html + '    <a onclick="forwardCall(' + value.column + ')">Weiterleiten ' + value.label + '</a>';
        }
      }
