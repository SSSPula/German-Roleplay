...
function showWinTeam(data, finished)
{
    ...
    if(data.win_team == "RedTeam") {
        $('#winning_team').html(`<span class="red">Die Strandgänger</span>`);
        //_textSpeak = "Die Strandgänger sind am drippen!";
        // Speak("Äá»™i Ä‘á» Ä‘Ã£ chiáº¿n tháº¯ng!");
    } else {
        $('#winning_team').html(`<span class="blue">Die Businessmänner</span>`);
        //_textSpeak = "Die Bissnesmänner machen käsh!";
        // Speak("Äá»™i xanh Ä‘Ã£ chiáº¿n tháº¯ng!");
    }
    $('#who_won').fadeIn();
    if(finished) {
        $('.div_info').html(`5 Sekunden noch <span class="red" id="remain_time"> </span> !`);
        //_textSpeak += " Das Spiel ist vorbei!";
    }
    Speak(_textSpeak);
...
function startMatch() {
    ...
    //Speak("Die Runde beginnt!");
}

function Speak(text) {
    $('#speak_ai').attr("src", "https://translate.google.com/translate_tts?ie=UTF-8&q="+encodeURI(text)+"&tl=de-us&total=1&idx=0&client=tw-ob");
    // $('#speak_ai').play();
