...
ESX.ShowInventory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
		...
		if data.current.canRemove then
			...
			table.insert(elements, {label = ('Gegenstand vorzeigen'), action = 'zeigen', type = data.current.type, value = data.current.value})
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									if type == 'item_weapon' then
										TriggerServerEvent('esx:gidewenspantossindnice1337veInventoryItem', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity and quantity > 0 and data.current.count >= quantity then
												TriggerServerEvent('esx:gidewenspantossindnice1337veInventoryItem', selectedPlayerId, type, item, quantity)
												ExecuteCommand("oocconsdstr hat etwas übergeben.")
												menu3.close()
												menu2.close()
												menu1.close()
											else
			...
			elseif data1.current.action == 'zeigen' then
			ExecuteCommand("oocconsdstr zeigt dir: " .. itemlabel)
			...
			elseif data1.current.action == 'remove' then
				if IsPedOnFoot(playerPed) and not IsPedFalling(playerPed) then
					...
					if type == 'item_weapon' then
						...
						ExecuteCommand("oocconsdstr hat " .. itemlabel .. " fallen gelassen.")
					else
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
							...
							if quantity and quantity > 0 and data.current.count >= quantity then
								...
								ExecuteCommand("oocconsdstr hat " .. quantity .. "x " .. itemlabel .. " fallen gelassen.")
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
						end, function(data2, menu2)
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										TriggerServerEvent('esx:gidewenspantossindnice1337veInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
