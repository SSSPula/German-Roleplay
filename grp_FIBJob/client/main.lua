...
function OpenfibActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fib_actions', {
		...
		elements = {
			...
			{label = ("🚗 Fahrzeug Interaktionen"), value = 'vehicle_interaction'},
		...
		if data.current.value == 'citizen_interaction' then
			...
			--	{label = _U('id_card'), value = 'identity_card'},
				...
				{label = ('👔 Umziehen'), value = 'umziehen'},
				--{label = ('Fallschirm auspacken'), value = 'fallschrrr'},
				{label = ('🖌️ Kennzeichen verschleiern'), value = 'kennzeichen'},
			--	{label = ('💼 Tasche wegschmeissen'), value = 'tascheweg'},
			--	{label = ('💊 Cyankali schlucken'), value = 'cyan'},
				{label = ('💳 Ausweis fälschen'), value = 'ausweisss'},
			--	{label = ('☁️ Es mysteriös gewittern lassen'), value = 'mysterioesesgewiddaalda'},
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					...
					elseif action == 'umziehen' then
						TriggerEvent('fibkobbb:kleiderldasdaauf')
						exports['mythic_notify']:DoHudText('inform', 'Du schaust nach deinen Wechsel-Outfits... Gute Vorbereitung führt zu tadellosen Ergebnissen...', { ['background-color'] = '#ffffff', ['color'] = '#000000' })

					elseif action == 'ausweisss' then
					TriggerEvent('grp_fhthdDFtz4r:1111122')
					exports['mythic_notify']:DoHudText('inform', 'Du holst dein Fälscher-Set heraus, nimmst den Ausweis in die Hand ... Es kann beginnen.', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
					elseif action == 'kennzeichen' then
						...
						if not DoesEntityExist(vehicle) then
							...
							exports['mythic_notify']:DoHudText('inform', 'Du hast das Kennzeichen deines Fahrzeuges zu: ' .. newPlate .. ' geändert!', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
						end
					...
					elseif action == 'cyan' then
						...
						exports['mythic_notify']:DoHudText('inform', 'Du nimmt eine Kapsel gefüllt mit Cyankali in den Mund... Der letzte Ausweg nicht entdeckt zu werden', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
						Citizen.Wait(3000)
						exports['mythic_notify']:DoHudText('inform', 'Das letzte was du spürst ist ein bitterer Geschmack hinten im Gaumen... Hoffentlich war es das wert...', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			table.insert(elements, {label = ('📧 Fahrzeug Extras'), value = "aendermeineextrasjonge"})
			table.insert(elements, {label = ("🔎 Kennzeichen abfragen"), value = 'search_database'})
...
RegisterNetEvent('fibkobbb:kleiderldasdaauf')
AddEventHandler('fibkobbb:kleiderldasdaauf', function()
KleiderSchrank()
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'fib' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBosdewenspantossindnice1337sMenu', 'fib', function(data, menu)
						menu.close()
