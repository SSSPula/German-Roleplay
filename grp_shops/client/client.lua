...
Citizen.CreateThread(function()
	...
	local vekauferrrdfib = {
        `s_m_m_fibsec_01`,
    }
	local vekauferrrddoj = {
        `hc_hacker`,
    }
	local sherffs = {
        `s_m_y_sheriff_01`,
    ...
    exports['bt-target']:AddTargetModel(kasseeddde, {
        options = {
            {
                event = "grp_shops:dasdofnnen",
    ...
    exports['bt-target']:AddTargetModel(waffekauddf, {
        options = {
            {
                event = "grp_shops:openpolddr",
                icon = "far fa-money-bill-alt",
                label = "Waffe kaufen",
            },
        },
        job = {"police", "hitman", "sheriff", "fib"},
        distance = 2.5
    })
	exports['bt-target']:AddTargetModel(sherffs, {
        options = {
            {
                event = "grp_shops:openpolddr",
                icon = "fas fa-shopping-basket",
                label = "Sheriff Shop öffnen",
            },
        },
        job = {"sheriff"},
        distance = 2.5
    })
	exports['bt-target']:AddTargetModel(vekauferrrddoj, {
        options = {
            {
                event = "grp_shops:dasdofnnen",
                icon = "fas fa-shopping-basket",
	...
	exports['bt-target']:AddTargetModel(vekauferrrdfib, {
        options = {
            {
                event = "grp_shops:dasdofnnen",
                icon = "fas fa-shopping-basket",
    ...
    exports['bt-target']:AddTargetModel(vekauferrrd, {
        options = {
            {
                event = "grp_shops:dasdofnnen",
                icon = "fas fa-shopping-basket",
...
RegisterNetEvent('grp_shops:openpolddr')
AddEventHandler('grp_shops:openpolddr', function()
...
RegisterNetEvent('grp_shops:dasdofnnen')
AddEventHandler('grp_shops:dasdofnnen', function()
for k,v in pairs(Config.Shops) do
			local distance = GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.cashier[1], v.cashier[2], v.cashier[3], false)
			if cashier_menu ~= nil then
				distance = GetDistanceBetweenCoords(coords.x, coords.y, coords.z, cashier_menu.cashier[1], cashier_menu.cashier[2], cashier_menu.cashier[3], false)
				while cashier_menu ~= nil and distance > 10.0 do cashier_menu = nil Citizen.Wait(1) end
				if cashier_menu == nil then ESX.UI.Menu.CloseAll() end
			else
				local mk = Config.MarkerSettings['cashier']
				if distance < 20.0 then
