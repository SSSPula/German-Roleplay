...
--[[
RegisterCommand('waffenskin',function()
...
--[[
RegisterCommand("debugskin", function(source)
	
	ESX.TriggerServerCallback('grrasdda:skdjfinskin', function (result)
		if result then
			if result[1].waffenskin == 1 then
				if result[1].skinstyle == 'Hillbilly' then
				SetWeaponAnimationOverride(PlayerPedId(), GetHashKey('Hillbilly'))
				elseif result[1].skinstyle == 'Gang1H' then
				SetWeaponAnimationOverride(PlayerPedId(), GetHashKey('Gang1H'))
				elseif result[1].skinstyle == 'default' then
				SetWeaponAnimationOverride(PlayerPedId(), GetHashKey('default'))
				end
				
				local tint = result[1].skintint
				for i = 1, #weapons do
				SetPedWeaponTintIndex(PlayerPedId(), tonumber(weapons[i].hash), tonumber(tint))
				end	
	...
	ESX.TriggerServerCallback('grrasdda:skdjfinskin', function (result)
		if result then
			if result[1].waffenskin == 1 then
				...
				if result[1].skinurl == '' or result[1].skinurl == 'aus' then
				...
				TriggerEvent('weapon:customURL',result[1].skinurl)
				end
...
RegisterNetEvent('grp_WS:menauf')
AddEventHandler('grp_WS:menauf', function()
...
RegisterNUICallback('sumbit', function(data, cb)
	...
	TriggerServerEvent('grp_561j4213hdd:uebergabte', tint, style, url)
	exports['mythic_notify']:DoHudText('inform', 'Einstellungen gespeichert', { ['background-color'] = '#ffffff', ['color'] = '#000000' })

	
end)

RegisterNetEvent('weapon:del3')
AddEventHandler('weapon:del3', function()
