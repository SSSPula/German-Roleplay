...
ctrl.update = function()    
    ...
    while true do      
        ...
        if inVeh then        
            if IsControlJustReleased(1, 800) then          
...
ctrl.setEngine = function(d,s)
    ...
    if GetIsVehicleEngineRunning(v) then
        ...
        exports['mythic_notify']:DoHudText('inform', 'Motor ausgeschaltet', 2500)
    else
        SetVehicleEngineOn(v, true, false, true)
        exports['mythic_notify']:DoHudText('inform', 'Motor eingeschaltet', 2500)
    end
end  
...
AddEventHandler('carcontrol:open', function()
    ...
    if inVeh then       
        local veh = GetVehiclePedIsIn(ped)
        exports['mythic_notify']:SendAlert('inform', 'Fahrzeugaktionen schließen: Rechtsklick!', 5500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
