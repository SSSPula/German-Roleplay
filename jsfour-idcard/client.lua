...
AddEventHandler('jsfour-idcard:shot', function(playerID)
	...
	local posx, posy = 0.832, 0.27
	local width, height = 0.05, 0.09
	local x, y = GetActiveScreenResolution()
	--print('x: '..x..' y: '..y) DEV SHIT
		if x == 1920 and y == 1080 then
			posx, posy = 0.775, 0.36
...
function openMenu()
	ESX.UI.Menu.Open(
	  'default', GetCurrentResourceName(), 'id_card_menu', {
		  --title    = 'ID menu',
		  title   = 'Ausweis und Waffenschein',
		  align    = 'top-left',
		  elements = {
			  {label = 'Personalausweis ansehen', value = 'checkID'},
			  {label = 'Personalausweis vorzeigen', value = 'showID'},
			  {label = 'Führerschein ansehen', value = 'checkDriver'},
			  {label = 'Führerschein vorzeigen', value = 'showDriver'},
			 --{label = 'Waffenschein ansehen', value = 'checkFirearms'},
			 --{label = 'Waffenschein vorzeigen', value = 'showFirearms'},
			  --{label = 'Dienstausweis vorzeigen', value = 'dienstausweis'},	
			  --{label = 'Dienstausweis ansehen', value = 'anschauendienstausweis'},			  
		  }
	  },
	  function(data, menu)
		  ...
		  elseif val == 'anschauendienstausweis' then
			  TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'weapon')
			  ...
			  if distance ~= -1 and distance <= 3.0 then
				  ...
				  elseif val == 'dienstausweis' then
			  		TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player), 'dienstausweis')
				  end
			  else
exports['mythic_notify']:DoHudText('inform', 'Niemand in der Nähe der das sehen könnte!', { ['background-color'] = '#ffffff', ['color'] = '#000000' })				
--ESX.ShowNotification('Niemand in der Nähe')
  ...
  RegisterNetEvent("jsfour-idcard:DewensKrasseMoeglichkeit") -- For opening the ID menu from another resource. lel
AddEventHandler("jsfour-idcard:DewensKrasseMoeglichkeit", function()
    openMenu()
end)

RegisterCommand("ausweiszeigen", function(source)
	local player, distance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
...
RegisterCommand("fszeigen", function(source)
	local player, distance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player), 'driver')
